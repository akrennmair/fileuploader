...
type PersistenceManager interface {
	WriteUploadProgress(upload_id string, percent int) error
	GetUploadProgress(upload_id string) (percent int, err error)
	OpenUpload(upload_id string) (io.ReadCloser, error)
	OpenUploadWritable(upload_id string) (io.WriteCloser, error)
	UploadExists(upload_id string) bool
	SaveUploadText(upload_id, text string) error
	GetUploadText(upload_id string) (string, error)
	SaveUploadFilename(upload_id, filename string) error
	GetUploadFilename(upload_id string) (string, error)
	GetUploadSize(upload_id string) (size int64, err error)
}

type FilePersistenceManager struct { }

func NewFilePersistenceManager() PersistenceManager {
...
func (f *FilePersistenceManager) WriteUploadProgress(upload_id string, percent int) error {
...
func (f *FilePersistenceManager) GetUploadProgress(upload_id string) (percent int, err error) {
...
func (f *FilePersistenceManager) OpenUpload(upload_id string) (io.ReadCloser, error) {
...
func (f *FilePersistenceManager) OpenUploadWritable(upload_id string) (io.WriteCloser, error) {
...
func (f *FilePersistenceManager) UploadExists(upload_id string) bool {
...
func (f *FilePersistenceManager) SaveUploadText(upload_id, text string) error {
...
func (f *FilePersistenceManager) GetUploadText(upload_id string) (string, error) {
	return slurp("files/" + upload_id + ".desc")
}

// this function saves the original filename for an upload ID. If the original filename
// has already been saved, the function returns an error.
func (f *FilePersistenceManager) SaveUploadFilename(upload_id, filename string) error {
...
func (f *FilePersistenceManager) GetUploadFilename(upload_id string) (string, error) {
	return slurp("files/" + upload_id + ".fn")
}

// this function returns the size of a previously uploaded file, identified
// by its upload ID.
func (f *FilePersistenceManager) GetUploadSize(upload_id string) (size int64, err error) {
